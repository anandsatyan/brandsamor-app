  ...
  <div className="space-y-12 sm:space-y-14">
    {sections.map((section) => (
      <section key={section.id} id={section.id} className="scroll-mt-28">
        <h2 className="type-h2-sm mb-4 text-heading">{section.title}</h2>
        {section.paragraphs.map((paragraph) => (
          <p key={paragraph} className="type-body mb-4 last:mb-0">
            {paragraph}
          </p>
        ))}
        {section.bullets && section.bullets.length > 0 && (
          <ul className="mt-4 space-y-2 list-disc pl-5 type-body">
...
export const ArticleFaq = ({ items }: { items: KbFaqItem[] }) => (
  <section id="faq" className="scroll-mt-28 border-t border-border pt-10 sm:pt-12">
    <h2 className="type-h2-sm mb-6 text-heading">Frequently asked questions</h2>
      ...
      {items.map((item) => (
        ...
        >
          <summary className="flex cursor-pointer list-none items-start justify-between gap-4 p-5 sm:p-6 [&::-webkit-details-marker]:hidden">
            <span className="type-h4 font-medium pr-2">{item.question}</span>
            <span className="text-accent type-h4 leading-none shrink-0" aria-hidden="true">
              +
            </span>
          </summary>
          <div className="px-5 sm:px-6 pb-5 sm:pb-6 -mt-1 type-body">{item.answer}</div>
        </details>
      ))}
    </div>
  </section>
...
export const RelatedKbArticles = ({ slugs, currentSlug }: { slugs: string[]; currentSlug?: string }) => {
  ...
  return (
    <section className="border-t border-border pt-10 sm:pt-12">
      <h2 className="type-h2-sm mb-6 text-heading">Related guides</h2>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {articles.map((article) => (
          ...
          >
            <p className="type-caption uppercase tracking-wider text-accent mb-2">{article.targetKeyword}</p>
            <h3 className="type-h4 font-medium leading-snug mb-2">{article.title}</h3>
            <p className="type-body-sm line-clamp-3">{article.excerpt}</p>
          </Link>
        ))}
      </div>
    </section>
...
const RelatedCommercialPages = ({ paths }: { paths?: string[] }) => {
  ...
  return (
    <section className="border-t border-border pt-10 sm:pt-12">
      <h2 className="type-h2-sm mb-6 text-heading">Related commercial pages</h2>
    ...
    <div className="min-h-screen bg-surface font-sans overflow-x-hidden">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 md:px-12 py-10 sm:py-14 pb-16 sm:pb-24">
        <header className="mb-10 sm:mb-12 max-w-3xl">
          ...
          <h1 className="type-h1 mb-5">{article.h1}</h1>
          <p className="type-body-lg mb-4">{article.excerpt}</p>
          <div className="type-body-sm text-body/80 space-y-1">
            <p>
