    ...
    <main className="max-w-3xl mx-auto px-4 sm:px-6 md:px-12 pb-16 sm:pb-24">
      <HeroPanel className="py-10 sm:py-14 mb-10 sm:mb-12 rounded-none sm:rounded-2xl text-center">
        <div className="mx-auto flex max-w-3xl flex-col items-center">
          {badge && (
            <span className="inline-block px-3 py-1 bg-white/20 text-white type-caption font-semibold uppercase tracking-wider rounded-full border border-white/30 mb-6">
              {badge}
            </span>
          )}
          <h1 className="type-h1 mb-6 text-white">{meta.h1}</h1>
          <p className="type-body-lg mb-0 max-w-2xl">{meta.description}</p>
        </div>
      </HeroPanel>
      {children}
    </main>
  ...
  <div className="space-y-10 sm:space-y-12">
    {sections.map((section) => (
      <section key={section.id} id={section.id} className="border-t border-border pt-8 sm:pt-10">
        <h2 className="type-h2-sm mb-4">{section.title}</h2>
