...
export const OptionCard = ({
  ...
  <button
    ...
    className={[
      ...
      selected
        ? 'sampling-option-selected border-[#FF600A] bg-[rgba(255,96,10,0.08)]'
        : 'border-[#EADFD3] bg-[#FFFDFC] hover:border-[#FF600A]/40',
      disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer',
      className,
    ].join(' ')}
  >
    {Icon && (
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FEF7ED] text-[#FF600A]">
    ...
    <span className={horizontal ? 'flex-1' : 'block'}>
      <span className="flex items-start justify-between gap-2">
        ...
        {selected && (
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#FF600A] text-white">
