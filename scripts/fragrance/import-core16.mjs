...
async function upsertTaxonomy(db, fragrances) {
  ...
  for (const f of fragrances) {
    ...
    for (const b of Array.isArray(f.bestForBusinessTypes) ? f.bestForBusinessTypes : []) businessTypeSlugs.add(String(b));
  }
...
async function ensureSuppliers(db, supplierPolicy) {
  const candidates = Array.isArray(supplierPolicy?.candidateSuppliers) ? supplierPolicy.candidateSuppliers : [];
...
function pyramidDocFromSource(f) {
  ...
  return compactObject({
    ...
    basis: notes.basis === 'original-reference' ? 'ORIGINAL_REFERENCE' : String(notes.basis ?? 'ORIGINAL_REFERENCE'),
    layers: {
    ...
    source: { kind: 'JSON_IMPORT', version: '1.0.0' },
    updatedAt: now(),
  });
}
...
async function upsertFragrance(db, f) {
  ...
  // Never overwrite supplier selection if it was manually set after import.
  if (existing?.sourcing?.selectedSupplier != null || existing?.sourcing?.activeSupplierVariantId != null) {
    // keep whatever is in DB
  } else if (f.sourcing) {
    update.$set.sourcing = {
    ...
    update.$set.sourcing = existing?.sourcing ?? { selectedSupplier: null, activeSupplierVariantId: null };
  }
...
async function upsertSupplierVariants(db, f) {
  ...
  for (const v of candidates) {
    ...
    // Do not overwrite manually-entered supplierProductCode with null from import.
    ...
    // Create compliance placeholders per supplier variant (do not mark as verified).
    const docs = [
    ...
    await db.collection('complianceDocuments').bulkWrite(
      docs.map((d) => ({
        updateOne: {
          ...
          update: { $setOnInsert: { supplierVariantId: variantId, type: d.type, status: d.status, createdAt: now() } },
          upsert: true,
        },
      })),
      { ordered: false },
...
async function main() {
  ...
  if (fragrances.length !== 16) {
    throw new Error(`Expected 16 fragrances in source, got ${fragrances.length}`);
  }
  ...
  console.log('✅ Imported Brandsamor Core 16 into MongoDB.');
}
